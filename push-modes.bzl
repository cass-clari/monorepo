PushModesProvider = provider(fields = ["type"])

modes = ["DEV", "INTEGRATION", "PROD"]
...
def _impl(ctx):
    ...
    raw_mode = ctx.build_setting_value
    ...
    if raw_mode not in modes:
        fail(str(ctx.label) + " build setting allowed to take values {" +
             ", ".join(modes) + "} but was set to unallowed value " +
             raw_mode)

    # Returns a provider like a normal rule
    return PushModesProvider(type = raw_mode)
...
def _mode_impl(ctx):
    ...
    print(ctx.attr._push_mode[PushModesProvider].type)
    return []

#breakfast = rule(
#    implementation = _mode_impl,
